...
if is_train:
    ...
    for epoch in range(max_epoch):
        for step, data in enumerate(data_loader):
            ...
            loss = loss_func(output_mu, target)

            loss.backward()
            optimizer.step()
            print("epoch{}, step{}, Loss={}".format(epoch, step, loss))
            iter += 1
            writer.add_scalar("loss_loc_dreg_mse", loss.item(), global_step=iter)
        if epoch % 100 == 0:
            torch.save(model.state_dict(), "model_loc_dreg_mse-epoch{}.pt".format(epoch))
    torch.save(model.state_dict(), "model_loc_dreg_mse-epoch{}.pt".format(epoch))
    ##############################################################################
else:
    # test
    with torch.no_grad():
        ...
        target_epoch = 1799
        para_file = "model_loc_dreg_mse-epoch{}.pt".format(target_epoch)
        model = SimpleMLP()
        model.cuda()
        ...
        tar_sum_y, pre_sum_y, inter_sum_y = get_discrete_distribution_diff(targets[:, 1], model_mu[:, 1], eval_seg)
        tar_sum_z, pre_sum_z, inter_sum_z = get_discrete_distribution_diff(targets[:, 2], model_mu[:, 2], eval_seg)
