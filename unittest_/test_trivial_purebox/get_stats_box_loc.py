...
from analysis.model_error_analysis.get_detection3d_error_stats import get_discrete_distribution_diff, get_error_segs
...
for step, data in enumerate(data_loader):
    eval_seg = get_error_segs(data['gt_box'], data['dt_box'])
...
if is_train:
    ...
    for epoch in range(max_epoch):
        for step, data in enumerate(data_loader):
            ...
            iter += 1
            writer.add_scalar("loss_loc", loss.item(), global_step=iter)
        if epoch % 50 == 0:
            torch.save(model.state_dict(), "model_loc-epoch{}.pt".format(epoch))
    torch.save(model.state_dict(), "model_loc-epoch{}.pt".format(epoch))
    ##############################################################################
else:
    # test
    with torch.no_grad():
        ...
        target_epoch = 1799
        para_file = "model_loc-epoch{}.pt".format(target_epoch)
        model = SimpleMLP()
        model.cuda()
        ...
        for step, data in enumerate(data_loader_test):
            ...
            model_mu = output_mu.cpu().numpy()
            model_logvar = output_logvar.cpu().numpy()



            tar_sum_x, pre_sum_x, inter_sum_x = get_discrete_distribution_diff(targets[:, 0], model_mu[:, 0], eval_seg[0])
            tar_sum_y, pre_sum_y, inter_sum_y = get_discrete_distribution_diff(targets[:, 1], model_mu[:, 1], eval_seg[1])
            tar_sum_z, pre_sum_z, inter_sum_z = get_discrete_distribution_diff(targets[:, 2], model_mu[:, 2], eval_seg[2])

            print("========== evaluation, epoch={}============".format(target_epoch))
            print("x_acc", sum(inter_sum_x) / sum(tar_sum_x))
            print("y_acc", sum(inter_sum_y) / sum(tar_sum_y))
            print("z_acc", sum(inter_sum_z) / sum(tar_sum_z))
