...
class Discriminator_MLP(nn.Module):
    def __init__(self, ngpu, input_size):
        ...
        self.fc1 = nn.Linear(input_size,15) # 第一个隐含层
        self.fc2 = nn.Linear(15,10) # 第二个隐含层
...
if __name__ == '__main__':
    ...
    image_size = 4  # 可以根据自己的需求改变，这里把图像缩成16个像素，Spatial size of training images. All images will be resized to this size using a transformer.
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(zip(dataloader_ori, dataloader_proc)): # dataloader_ori存放原图，用于训练G；dataloader_proc存放缩减图，处理后用于训练D
            ...
            output = netDMLP(fake.detach()).view(-1)        # detach作用？？？
            ...
            # Save the Best Model
            if errG < loss_tep:
                torch.save(netG.state_dict(), './results/model.pt')
                loss_tep = errG
