...
def get_gtbbox_gen_fpbbox():
    ...
    for img_id in range(0,7481):
        ...
        index_2d = [True, True, False, True, True, False ,True] #用于转换3d-2d
        gt_bboxes = []
        fp_bboxes_all = []
        fp_bboxes_easy = []
        fp_bboxes_hard = []
        ...
        for dt_i in range(num_dt): # 处理第dt_i个检测框数据
            if dt_i not in db['3d'][img_id]:
                fp_box_lidar = dt_annos[img_id]['boxes_lidar'][dt_i][index_2d]
        ...
        fp_bboxes_all = fp_bboxes_all[:100] + [0,]*(100-len(fp_bboxes_all)) # 将每幅图像的FPbbox输出固定为20个，即最后为20*5=100维，在训练时补零项不参与梯度回传
        fp_bboxes_easy = fp_bboxes_easy[:100] + [0,]*(100-len(fp_bboxes_easy)) # 将每幅图像的easyFPbbox输出固定为20个，即最后为20*5=100维
        fp_bboxes_hard = fp_bboxes_hard[:50] + [0,]*(50-len(fp_bboxes_hard)) # 将每幅图像的hardFPbbox输出固定为10个，即最后为10*5=50维
    ...
    with open("D:/1Pjlab/ADModel_Pro/data/gtbbox_gen_20fpbbox_2d.pkl", "wb") as f:
