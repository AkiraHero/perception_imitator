...
class OpenPCDetPVRCNN(ModelBase):
    ...
    '''
    This embedding model output results directly in data batch dict, with the following keys:
        'voxel_features', 
    ...
    def post_processing(self, batch_dict):
        ...
        """
        post_process_cfg = self.embedding_model.model_cfg.POST_PROCESSING
        ...
        for index in range(batch_size):
            ...
            if not isinstance(batch_dict['batch_cls_preds'], list):
                ...
                assert cls_preds.shape[1] in [1, self.embedding_model.num_class]
            ...
            if post_process_cfg.NMS_CONFIG.MULTI_CLASSES_NMS:
                if not isinstance(cls_preds, list):
                    cls_preds = [cls_preds]
                    multihead_label_mapping = [torch.arange(1, self.embedding_model.num_class, device=cls_preds[0].device)]
                else:
