...
def draw_agent_boxes(center_agent_annotation: Dict[str, Any],
    ...
    for instance_token, annotations in agent_history.items():
        ...
        for i, annotation in enumerate(annotations):
            ...
            if instance_token == center_agent_annotation['token']:
                color = (255, 0, 0)
            else:
...
class AgentBoxesWithFadedHistory(AgentRepresentation):
    ...
    def make_representation(self, ego_pose_token: str, sample_token: str) -> np.ndarray:
        """
        Draws agent boxes with faded history into a black background.
        :param ego_pose_token: ego pose token.
        ...
        center_agent_annotation = self.helper.get_ego_pose_info(ego_pose_token)
