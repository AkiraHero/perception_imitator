...
class TpFpSceneOccHeatmapDataset(DatasetBase):
    def __init__(self, config):
        ...
        if self._distribution_setting == True:
            ...
            tp_distribution_name = os.path.join(self._data_root, "tp_distribution.pkl")     # 存储的是所有实验得到TP的各个参数的均值和方差
            with open(tp_distribution_name, 'rb') as f:
                self._tp_distribution = pickle.load(f)   # 存储的是所有实验得到TP的各个参数的均值和方差
    ...
    def get_data_loader(self, distributed=False):
        return DataLoader(
            ...
            collate_fn=TpFpSceneOccHeatmapDataset.collate_batch
        )
    ...
    def update_label_map(self, map, bev_corners, reg_target, type):     # type表示框类型，0为tp，1为fp
        ...
        for p in points:
            ...
            map[label_x, label_y, type] = 1.0
            if self._distribution_setting == True:
                map[label_x, label_y, 2:13] = actual_reg_target
            else:
                map[label_x, label_y, 2:8] = actual_reg_target
    ...
    def get_label(self, idx):
        if self._distribution_setting == True:
            label_map = np.zeros((self._geometry['label_shape'][0], self._geometry['label_shape'][1], 13), dtype=np.float32)
        else:
            label_map = np.zeros((self._geometry['label_shape'][0], self._geometry['label_shape'][1], 8), dtype=np.float32)
        label_list = [[] for _ in range(2)]

        if self._distribution_setting == True:
            # 处理TP       
            tp_bboxes = self._tp_distribution['tp_mean'][idx]   # 将均值作为期望的被检测框参数
            tp_var = self._tp_distribution['tp_var'][idx]
            tp_num = self._tp_distribution['tp_num'][idx]       # TP可以不用筛选，因为其已经和GT使用IoU对应过
            tp_scores = self._tp_distribution['tp_score'][idx]

            for i in range(len(tp_bboxes)):     # 处理一个TP框
                if tp_num[i] >= 10 and tp_scores[i] >= 0.3:
                    x = tp_bboxes[i][0]
                    y = tp_bboxes[i][1]
                    l = tp_bboxes[i][3]
                    w = tp_bboxes[i][4]
                    ...
                    self.update_label_map(label_map, corners, reg_target, type=0)
                    label_list[0].append(corners)
            # 处理FP
            ...
            fp_scores = self._fp_distribution['fp_score'][idx]

            for i in range(len(fp_bboxes)):     # 处理一个FP框
                if fp_num[i] >= 50 and fp_scores[i] >= 0.3:
                    x = fp_bboxes[i][0]
                    y = fp_bboxes[i][1]
                    l = fp_bboxes[i][3]
                    w = fp_bboxes[i][4]
                    ...
                    corners, reg_target = self.get_corners([x, y, l, w, yaw, x_var, y_var, l_var, w_var, yaw_var], use_distribution=True)
                    self.update_label_map(label_map, corners, reg_target, type=1)
                    label_list[1].append(corners)      
