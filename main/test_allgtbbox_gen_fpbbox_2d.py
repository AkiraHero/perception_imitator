...
from utils.IoU_calculate import boxes_iou3d_cpu, cal_overlaps_bev
...
if __name__ == '__main__':
    ...
    paras = torch.load("D:/1Pjlab/ADModel_Pro/output/gtbbox_gen_20fpbbox_model_2d/400.pt")
    ...
    with torch.no_grad():
        for step, data in enumerate(data_loader):
            ...
            dt_box_fp = dt_box_fp.view(dt_box_fp.shape[0], -1, 5)
            gen_data = gen_data.view(dt_box_fp.shape[0], -1, 5)
    ...
    for i in range(5,6):
        ...
        overlaps_bev = torch.FloatTensor(torch.Size((gen_fp_bbox.shape[0], gt_fp_bbox.shape[0]))).zero_()
        iou_2d = cal_overlaps_bev(gen_fp_bbox[i], gt_fp_bbox[i],overlaps_bev)
        all_iou.append(iou_2d)

    all_iou = list_of_tensor_2_tensor(all_iou)
    print("IoU_2D is:", all_iou)    # 目前高度的生成可能存在问题，导致3D检测框的IoU值为0，但是从bev视角看，是可以生成有重合部分的FP框的
