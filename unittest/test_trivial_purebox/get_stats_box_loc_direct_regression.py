...
if is_train:
    ...
    for epoch in range(max_epoch):
        for step, data in enumerate(data_loader):
            ...
            iter += 1
            writer.add_scalar("loss_loc_dreg", loss.item(), global_step=iter)
        if epoch % 50 == 0:
            torch.save(model.state_dict(), "model_loc_dreg-epoch{}.pt".format(epoch))
    torch.save(model.state_dict(), "model_loc_dreg-epoch{}.pt".format(epoch))
    ##############################################################################
else:
    # test
    with torch.no_grad():
        ...
        target_epoch = 1799
        para_file = "model_loc_dreg-epoch{}.pt".format(target_epoch)
        model = SimpleMLP()
        model.cuda()
